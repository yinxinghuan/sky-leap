...
- `game.js`：Three.js 场景、跳跃物理、命中判定、音效、56 位角色实例替换、动物专属动作和分数提交。
- `src/character-library.js`：共享正式角色库的 56 个 GLB 资产 URL、目录元数据、预载/三次重试和可释放克隆工厂。
- `src/assets/characters/`：从共享角色库同步的 56 个正式 GLB，按 people / archetypes / monsters / animals / office / villains 等来源保留文件名。
...
- 角色选择：`startGame({ selectedCharacter })` 接收已装备角色；`setCharacter()` 只接受 56 项 `CHARACTER_CATALOG` 中的 key，并且在待机/结算时立刻重建角色网格。`preloadCharacterLibrary()` 完成后会将当前后备角色替换为对应正式 GLB；`reset()` 始终重用当前装备，不再随机抽取角色。
...
- 收藏存档：`index.html` 的 `sl.collection.v1` 保存 `{ tickets, unlocked, equipped }`。初始角色为 `commuter`；结算时 `awardTickets()` 按分数增加 5–25 张车票；购买成功后立即写回并装备。角色仓库可筛选全部/人物/怪物/动物，并显示 `收藏 X / 56`。`setLivePreview()` 将当前 GLB 装入商店专用 Three.js 画布，`renderLivePreview()` 在商店打开时以 requestAnimationFrame 缓慢转身、浮动并在关闭时停止循环；`thumbFor()` 只为两侧相邻角色生成缓存 PNG。
...
- 调整商店视觉、响应式布局或状态：编辑 `index.html` 中 `#shop`、`.shop-sheet`、`.shop-stage` 和相关 CSS 变量；角色舞台会在 165–350 px 之间弹性收缩，短宿主高度在 720 px 以内启用紧凑间距，并同步更新 `doc/visual.md`。
